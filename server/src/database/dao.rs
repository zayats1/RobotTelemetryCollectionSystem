...
impl DAO for MovementInfo {
    async  fn insert_to_db(&self, conn: &Connection) -> libsql::Result<u64> {
        conn.execute(
            "INSERT INTO  MovementInfo(id,robot_type) VALUES (?, ?)",
            params![self.id.clone(), self.speed.x,self.speed.y,self.speed.z,self.acc.x,self.acc.y,self.acc.z,self.timestamp.to_string()],
        ).await
    }
    ...
    where
        Self: Sized,
    {
        let stmt = conn.prepare("SELECT id, robot_type FROM  Movement WHERE id = (?)").await?;
        let mut rows = stmt
            .query([id])
            .await?;
...
fn sql_val_to_f32(row: &Row, idx:i32) -> Result<f32, Error> {
    row.get_str(1)?.parse::<f32>().map_err(|e| libsql::Error::InvalidParserState(e.to_string()))
}
